-- Chasm Bridge Charity Website Review v1
-- Additive, idempotent migration.
-- DO NOT RUN AUTOMATICALLY. Review first, then run once in Supabase SQL Editor.
--
-- Adds a versioned guided-review template for the Chasm Bridge Charity website:
--   template-chasm-bridge-website-review-v1
--
-- The ordered 31-section website inventory remains in
...
-- gates so Chasm Bridge Charity Website Review v1 requires exactly 31
-- reviewed entries before submission.
--
...
--   Chasm Bridge Charity Website v1     -> 31

-- =============================================================================
-- 1. CHASM BRIDGE CHARITY WEBSITE REVIEW v1 TEMPLATE ROW
-- =============================================================================
INSERT INTO client_input_templates (id, title, description)
VALUES (
  'template-chasm-bridge-website-review-v1',
  'Chasm Bridge Charity Website Review',
  'Structured section-by-section review of the Chasm Bridge Charity website (31 visible website sections).'
)
ON CONFLICT (id) DO NOTHING;

-- =============================================================================
-- 2. CHASM BRIDGE CHARITY WEBSITE REVIEW v1 STRUCTURED SECTIONS
-- =============================================================================
...
SELECT * FROM (VALUES
  ('template-chasm-bridge-website-review-v1', 'current_concern', 'Corrected Wording', 'Long Text', 'Correct wording, names, facts, programme details or claims for this website section.', 10, false, NULL::jsonb),
  ('template-chasm-bridge-website-review-v1', 'remove_content', 'Information to Remove', 'Long Text', 'Information, people, visuals, form fields or claims that should be removed.', 20, false, NULL::jsonb),
  ('template-chasm-bridge-website-review-v1', 'replace_content', 'Information to Add', 'Exact Copy', 'Information or wording that should be added.', 30, false, NULL::jsonb),
  ('template-chasm-bridge-website-review-v1', 'copy_treatment', 'Logo or Branding Change', 'Long Text', 'Logo, brand, colour or visual identity feedback.', 35, false, NULL::jsonb),
  ('template-chasm-bridge-website-review-v1', 'visual_direction', 'Image or Photograph Change', 'Long Text', 'Image, photograph, diagram, profile picture or visual feedback.', 40, false, NULL::jsonb),
  ('template-chasm-bridge-website-review-v1', 'structure_changes', 'Layout or Section Order Change', 'Long Text', 'Layout, order, pathway or section-structure feedback.', 50, false, NULL::jsonb),
  ('template-chasm-bridge-website-review-v1', 'additional_comments', 'Additional Comments', 'Long Text', 'Button, link, contact detail, form flow or any other feedback Embark should know.', 60, false, NULL::jsonb)
...
WHERE NOT EXISTS (
  ...
  WHERE s.template_id = 'template-chasm-bridge-website-review-v1' AND s.section_key = v.section_key
);
